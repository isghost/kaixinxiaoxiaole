...
export default class GameModel {
   ...
   * @returns {([]|string|*)[]}
   */
  checkPoint(x: number, y: number, recursive?: boolean): [Vec2[], string, number, Vec2] {
    ...
    let result: [Vec2[], string, number, Vec2] = [samePoints, newCellStatus, this.cells[y][x]!.type!, new Vec2(x, y)];
  ...
  checkWithDirection(x: number, y: number, direction: Vec2[]): Vec2[] {
    ...
    while (front < queue.length) {
      //let direction = [new Vec2(0, -1), new Vec2(0, 1), new Vec2(1, 0), new Vec2(-1, 0)];
  ...
  getCells(): Array<Array<CellModel | null>> {
    return this.cells;
  }
  ...
  processBomb(bombModels: CellModel[], cycleCount: number) {
    while (bombModels.length > 0) {
      ...
      bombModels.forEach(function (this: GameModel, model: CellModel) {
  ...
  addWrapBomb(playTime: number, pos: Vec2) {
    // TODO
  }
